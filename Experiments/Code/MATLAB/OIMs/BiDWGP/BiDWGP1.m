function [osi, SNR_M, MLD] = BiDWGP1(srcs, fs, varargin)
...
%                                   e.g., BiDWGP1(srcs, fs, 'location',[0, 30, -60]).
...
%                                   e.g., BiDWGP1(srcs, fs, 'distance',[2, 3, 1.5]).
...
% 
% usage: osi = BiDWGP1([speech, masker1, masker2], fs, ...
%                    'location',[30, 90, -45],...
%                    'distance',[2],...
%                    'mode','B')
...
if ~strcmp(mode, 'M')
   ...
   if strcmp(mode, 'B') % if binaural listening
      ...
      % using approach in Culburn, 1977
      MLD = zeros(nchans, 1);
      for i = 1:nchans
