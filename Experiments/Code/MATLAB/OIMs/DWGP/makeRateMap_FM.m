function [ratemap,cf] = makeRateMap_FM(x,fs,lowcf,highcf,numchans,frameshift,ti,compression, outermiddle, reflevel)
% [ratemap,cf] = makeRateMap(x,fs,lowcf,highcf,numchans,frameshift,ti,compression, outermiddle, reflevel) 
% generates the spectro-temporal excitation pattern (STEP) representation for given signal. 
% This is to model smoothed and compressed representation of the envelope of the basilar membrane 
% response to sound. The wavform is was initially processed by gammatone filterbankds using a 
% pole-mapping procedure described in (Cooke, 1993). The Hilbert envelope in each channel of the 
% filterbank was computed and smoothed with a leaky integrator with 8 ms (in default) time constant 
% (Moore et al., 1988). The smoothed envelope was further downsampled.
%
% input:
%        x             input signal
...
switch lower(outermiddle)
    ...
    case 'terhardt'
        gain = gain .* db2amp(-terhardt_threshold(cf), reflevel);
    case 'none'
end
...
for c=1:numchans
    ...
    env = gain(c)*abs(u);
    smoothed_env = intgain .* filter(1,[1 -intdecay],env);         % temporal integration
    env_fm = forwardMasking2(amp2db(smoothed_env, 2e-5), 200, 20, fs);
    tmp = mean(reshape(env_fm,frameshift_samples,numframes)); % downsampling
...
% equipment. Part 7: Reference threshold of hearing under free-filed and diffuse-field listening 
% conditions. Note, only defined for frequencies between 20 and 12,500 Hz.
%
% input:
%        cfs       a vector of central frequencies in Hz
% output:
...
function level = amp2db(amp, ref)
level = 20 * log10(amp./ref);
