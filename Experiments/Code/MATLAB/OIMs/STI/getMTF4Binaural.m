function MTF = getMTF4Binaural(en_s, en_sn, MFs)
fs = 30;
fr_max = round(fs/2);
...
psd_s = (1/(fs*len2_sig)) .*  (abs(spc_s) .* conj(spc_s)); % to get actuall PSD: normalised by 2 * (1/(fs*N))

spc_sn = fft(en_sn);
spc_sn = spc_sn(1:len2_sig);
% psd_sn = (1/(fs_downsampled*len2_sig)) * abs(spc_sn).^2;
psd_sn = (1/(fs*len2_sig)) .* (abs(spc_s) .* conj(spc_sn));
...
% MTF = mui * real(EN_sn./ EN_s);   % Eqn.B1 in Drullman et al. 1994
