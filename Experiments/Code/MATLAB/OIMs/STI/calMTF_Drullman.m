function MTF = calMTF_Drullman(band_s, band_sn, fs, fs_downsampled, MFs)
...
% fixed when the duration of the signal is too small, leading to the first
% modulation band being 0
MFs_sample_l = max(round(MFs(:, 1) .* len2_sig ./ fr_max), 1); % get correponding sample points for each MF
