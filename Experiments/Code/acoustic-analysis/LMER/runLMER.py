...
def runLMER(df, formula, printList, which, step, level, segmentalModel, ttype, random_intercept, response_variable, print = True):
...
def imputeMissingData(df):
    ...
    for conditionDF in [fifteen, twenty, twentyfive, thirty]:
        ...
        valuesDF = conditionDF.loc[:, df.columns[3]:df.columns[-3]]
...
def loadDataSet(level, which, segmentalModel):
    ...
    df1 = pd.read_csv("./{}/GlobalMeasures_{}-categorical.csv".format('global', which))
    condition = df1[['ID', 'Condition']]
    df = pd.merge(df, condition, on = ['ID'])
...
def main(level = "segmental", ttype = 'categorical', which = "Full_wave_enhanced_audio", segmentalModel = "Phoneme_Category-vowel_dur_categories", step = False):
    ...
    if segmentalModel == "Vowel_Space":
        print(1)
        # NOTE calculate correlations and report anything significant (though it's unexpected)
        

    # http://web.pdx.edu/~newsomj/mlrclass/ho_randfixd.pdf
    fixed_variables = ['Condition', 'MMSE', 'Gender']
    random_variable = ['Age']
    random_intercept = ['ID']
    ...
    for response_variable in response_variables:
        ...
        formula = stepwise.main(df, response_variable, fixed_variables, random_variable, random_intercept, generalized = False)

        # Run LMER with BIC
        printList = list()
        printList.append("Level:    {}      Which:  {}      Formula:  {}".format(level, which, formula))
        print(printList)
        runLMER(df, formula, printList, which, step, level, segmentalModel, ttype, random_intercept, response_variable)

        # Run LMER with ALL
        printList = list()
        printList.append("Level:    {}      Which:  {}      Formula:  {}".format(level, which, formula))
        formula = "{} ~ {} + (1|{})".format(response_variable, ' + '.join(fixed_variables), random_variable[0])
        runLMER(df, formula, printList, which, False, level, segmentalModel, ttype, random_intercept, response_variable)
